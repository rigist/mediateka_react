...
function App({ children }) {
  ...
  return (
    <div
      style={{ backgroundColor: "green", margin: 3, padding: 2, height: 500 }}
    >
      <ul>
        <Navigation />
      </ul>
  ...
  return (
    <div>
      <input
        ...
        onChange={(event) =>
          changeItem(name, event, editId, setItems, items, setObj, obj)
        }
      />
    </div>
  );
...
function ButtonSave({
  ...
  return (
    <div>
      <button
        onClick={() =>
          saveItem(editId, setEditId, setItems, items, obj, setObj, getInitObj)
        }
      >
        save
      </button>
